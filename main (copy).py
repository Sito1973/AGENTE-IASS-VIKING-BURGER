...
def generate_response(api_key, message, assistant_content, thread_id, event, subscriber_id, valor_cache):
    ...
    try:
        ...
        # Preparar el mensaje del usuario con o sin cache_control basado en valor_cache
        ...
        # Agregar cache_control a mensajes de usuario anteriores si valor_cache == 1
        if valor_cache == 1:
            conversation_history = add_cache_control_to_user_messages(conversation_history)
        ...
        # Preparar assistant_content con o sin cache_control basado en valor_cache
        ...
        if valor_cache == 1:
            assistant_content_blocks[0]["cache_control"] = {"type": "ephemeral"}

        # Siempre usar el método beta.prompt_caching.messages.create
        create_message_method = client.beta.prompt_caching.messages.create

        # Iniciar la interacción con el modelo de Anthropic
        while True:
            response = create_message_method(
                model="claude-3-5-sonnet-20241022",
                max_tokens=1000,
                temperature=0.8,
                system=assistant_content_blocks,
            ...
            # Almacenar los tokens en el diccionario conversations
            conversations[thread_id]["input_tokens"] = input_tokens
            conversations[thread_id]["output_tokens"] = output_tokens

            logger.info("Tokens utilizados - Input: %d, Output: %d", input_tokens, output_tokens)
            ...
            if response.stop_reason == "tool_use":
                ...
                logger.debug("Entrada de la herramienta: %s", tool_input)
                ...
                if tool_name in tool_functions:
                    ...
                    logger.debug("Resultado de la herramienta %s añadido al historial", tool_name)
                else:
...
def add_cache_control_to_user_messages(conversation_history):
    for message in conversation_history:
        if message['role'] == 'user':
            # Recorrer los bloques de contenido
            for content_block in message['content']:
                if content_block.get('type') == 'text':
                    # Agregar cache_control si no está presente
                    if 'cache_control' not in content_block:
...
def send_message():
    ...
    valor_cache = data.get('valor_cache', 0)   # Obtener valor_cache, por defecto 0 si no está presente

    logger.info("valor_cache recibido: %s", valor_cache)
    ...
    if thread_id not in conversations:
        conversations[thread_id] = {
            "status": "processing",
            "response": None,
            "messages": [],  # Inicializar la lista de mensajes
    ...
    # Iniciar el hilo para generar la respuesta, pasando valor_cache
    thread = Thread(target=generate_response, args=(
        api_key, message, assistant_content, thread_id, event, subscriber_id, valor_cache))
    ...
    if conversations[thread_id]["status"] == "completed":
        ...
        return jsonify({
            ...
            "thread_id": thread_id,
            "input_tokens": conversations[thread_id].get("input_tokens"),
            "output_tokens": conversations[thread_id].get("output_tokens")
        })
    else:
...
def check_status():
    ...
    while time.time() - start_time < 8:
        ...
        if status == "completed":
            ...
            return jsonify({"response": conversations[thread_id]["response"]})
        elif status == "error":
