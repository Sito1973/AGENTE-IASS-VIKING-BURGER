...
def generate_response(api_key, message, assistant_content_text, thread_id, event, subscriber_id, use_cache_control):
    ...
    try:
        ...
        while True:
            # Antes de llamar a la API, verificar que el último mensaje es del usuario
            if conversation_history[-1]["role"] != "user":
                logger.error("La última entrada en el historial no es del usuario. No se puede continuar.")
                conversations[thread_id]["response"] = "Error interno: La conversación no está en un estado válido."
            ...
            if response.stop_reason == "tool_use":
                ...
                logger.info("Contenido completo de la respuesta del asistente: %s", response.content)
                ...
                if tool_name in tool_functions:
                    ...
                    logger.info(f"Resultado de la herramienta {tool_name} ANTES de json.dumps(): {result}")  # Nuevo log
                    result_json = json.dumps(result)
                    logger.info(f"Resultado de la herramienta {tool_name} DESPUÉS de json.dumps(): {result_json}")  # Nuevo log

                    # Agregar el resultado de la herramienta al historial de conversación como mensaje del usuario
                    conversation_history.append({
                        "role": "user",
                        "content": [
                            {
                    ...
                    logger.info(f"Contenido del mensaje enviado a Anthropic: {conversation_history[-1]}")
                    logger.debug("Resultado de la herramienta %s añadido al historial", tool_name)

                    # Continuar el bucle para realizar otra llamada al modelo
                    continue
                else:
