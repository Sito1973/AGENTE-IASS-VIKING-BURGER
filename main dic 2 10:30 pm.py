...
ASSISTANT_FILES = {
    1: '1 ASISTENTE_PRINCIPAL.TXT',
    2: 'ASISTENTE_CLIENTES_EXISTENTES.txt',
    3: 'ASISTENTE_EXTRACTOR_DATOS_NUEVO.TXT',
    4: 'ASISTENTE_EXTRACTOR_DATOS_EXISTENTE.TXT',
    5: "ASISTENTE_PRINCIPAL_TOOLS.TXT"
}
...
class ManyChatAPI:
    def __init__(self):
        self.base_url = 'https://api.manychat.com/fb'
        self.api_key = os.environ.get('MANYCHAT_API_KEY', '2009465:bbd96efc570bd145faa7171b593278e7')
    ...
    def set_custom_fields(self, subscriber_id, contact_data):
        ...
        logger.debug("Enviando set_custom_fields para subscriber_id: %s", subscriber_id)
        ...
        logger.info("Respuesta set_custom_fields: %s %s", response.status_code, response.text)
        return response
    ...
    def send_flow(self, subscriber_id, flow_ns="content20241029204321_898129"):
        ...
        response = requests.post(url, headers=self.headers, json=payload)
        logger.info("Respuesta send_flow: %s %s", response.status_code, response.text)
        return response
    ...
    def set_custom_fields_update(self, subscriber_id, contact_data):
        ...
        logger.debug("Enviando set_custom_fields_update para subscriber_id: %s", subscriber_id)
        url = f'{self.base_url}/subscriber/setCustomFields'
        payload = {
            "subscriber_id": subscriber_id,
            "fields": [
        ...
        logger.debug("Enviando set_custom_fields_update con payload: %s", payload)
        response = requests.post(url, headers=self.headers, json=payload)
        logger.info("Respuesta set_custom_fields_update: %s %s", response.status_code, response.text)
        return response

    def send_flow_update(self, subscriber_id, flow_ns="content20241120044926_071414"):
        ...
        response = requests.post(url, headers=self.headers, json=payload)
        logger.info("Respuesta send_flow_update: %s %s", response.status_code, response.text)
        return response
...
def save_contact(contact_data, subscriber_id):
    ...
    try:
        ...
        fields_response = manychat.set_custom_fields(subscriber_id, contact_data)
        ...
        result = {
            "status": "success" if flow_response.status_code in [200, 201] else "partial_success",
            "message": "Proceso completado exitosamente" if flow_response.status_code in [200, 201] 
                      else "Campos guardados pero error al enviar flow",
            "fields_response": fields_response.json(),
            "flow_response": flow_response.json() if flow_response.status_code in [200, 201] else None,
            "data": contact_data
        }
...
def crear_link_pago(tool_input, subscriber_id):
    ...
    try:
        ...
        if flow_response.status_code in [200, 201]:
            result = {
                "status": "success",
                "message": "Enlace de pago creado y flujo enviado exitosamente",
...
def update_contact(contact_data, subscriber_id):
    ...
    try:
        ...
        fields_response = manychat.set_custom_fields_update(subscriber_id, contact_data)
        if fields_response.status_code not in [200, 201]:
            logger.error("Error al actualizar campos: %s", fields_response.text)
            return {
                "status": "error",
                "message": f"Error al actualizar campos: {fields_response.text}",
                "status_code": fields_response.status_code
            }
        ...
        if flow_response.status_code not in [200, 201]:
            ...
            return {
                ...
                "message": f"Campos actualizados pero error al enviar flujo: {flow_response.text}",
                "fields_response": fields_response.json(),
...
# Configurar el logger si aún no lo has hecho
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Suponiendo que tienes un diccionario global para manejar las conversaciones
conversations = {}

# Aquí se definen las funciones de las herramientas
def save_contact(tool_input, subscriber_id):
    # Implementación de la herramienta 'crear_contacto'
    pass

def crear_link_pago(tool_input, subscriber_id):
    # Implementación de la herramienta 'crear_link_pago'
    pass

def update_contact(tool_input, subscriber_id):
    # Implementación de la herramienta 'actualizar_contacto'
    pass

def generate_response(api_key, message, assistant_content_text, thread_id, event, subscriber_id, use_cache_control):
    ...
    try:
        ...
        conversation_history = conversations[thread_id]["messages"].copy()

        # Función para enviar mensajes y manejar la respuesta
        def send_message_block(new_message_block, conversation_history):
            ...
            logger.debug("Respuesta de Anthropic: %s", response)
            ...
            # Procesar el uso de herramientas si es necesario
            messages = check_and_use_tool(messages, response)
            return messages, response

        # Función para verificar y usar herramientas
        def check_and_use_tool(messages, response):
            if response.stop_reason == "tool_use":
                ...
                if tool_name in tool_functions:
                    ...
                    logger.debug("Resultado de la herramienta %s: %s", tool_name, result)
                    result_json = json.dumps(result)
                    logger.info(f"Resultado de la herramienta {tool_name} convertido a JSON: {result_json}")

                    # Enviar el resultado de la herramienta al asistente
                    messages, _ = send_tool_result(result_json, tool_use_content.id, messages)
                else:
                    logger.warning("Herramienta desconocida: %s", tool_name)
                    conversations[thread_id]["response"] = f"Herramienta desconocida: {tool_name}"
                    conversations[thread_id]["status"] = "error"
            return messages

        # Función para enviar el resultado de la herramienta
        def send_tool_result(tool_result, tool_use_id, conversation_history):
            tool_response_block = {
                "role": "user",
                "content": [
                    {
            ...
            return send_message_block(tool_response_block, conversation_history)
        ...
        tool_functions = {
            ...
            "actualizar_contacto": update_contact
        }
        ...
        # Crear el bloque de mensaje del usuario
        ...
        conversation_history, response = send_message_block(user_message_block, conversation_history)
        ...
        logger.info("Cache Creation Input Tokens: %d", usage["cache_creation_input_tokens"])
        logger.info("Cache Read Input Tokens: %d", usage["cache_read_input_tokens"])

        # Construir la respuesta final del asistente
...
def send_message():
    ...
    try:
        ...
        logger.warning("El 'subscriber_id' proporcionado no es válido: %s", subscriber_id)
        return jsonify({"error": "El 'subscriber_id' proporcionado no es válido"}), 400
    ...
    try:
        ...
        logger.debug("Contenido del asistente procesado con variables: %s", assistant_content)
    except Exception as e:
    ...
    if thread_id not in conversations:
        ...
        if assistant_value is not None:
            ...
            logger.debug("Actualizado valor de assistant para thread_id: %s", thread_id)
        else:
    ...
    thread = Thread(target=generate_response, args=(
        api_key, message, assistant_content, thread_id, event, subscriber_id, use_cache_control))
    logger.info("Iniciando hilo con subscriber_id: %s", subscriber_id)
    thread.start()
    logger.info("Thread iniciado para generar respuesta en thread_id: %s", thread_id)

    event.wait(timeout=8)
    logger.debug("Esperada respuesta para thread_id: %s", thread_id)
    # Logs adicionales para seguimiento
    logger.debug("Estado de la conversación para thread_id %s: %s", thread_id, conversations[thread_id]["status"])
    logger.debug("Respuesta del asistente para thread_id %s: %s", thread_id, conversations[thread_id]["response"])
...
def check_status():
    ...
    while time.time() - start_time < 8:
        ...
        if status == "completed":
            ...
            logger.error("Estado error para thread_id: %s, mensaje: %s", thread_id, conversations[thread_id]['response'])
            return jsonify({"response": f"Error: {conversations[thread_id]['response']}"})
        time.sleep(0.5)
...
def letra_nombre():
    ...
    for resolucion in resoluciones:
        base64_img, svg_code = create_svg_base64(first_letter, resolucion, resolucion)
...
def upload_file():
    ...
    if response.status_code == 200:
        ...
        logger.error("No se pudo descargar el archivo desde la URL, status_code: %s", response.status_code)
    ...
    FRESHSALES_API_KEY = os.environ.get('FRESHSALES_API_KEY', "TU_FRESHSALES_API_KEY_AQUI")
    FRESHSALES_BASE_URL = os.environ.get('FRESHSALES_BASE_URL', 'https://tu_dominio.myfreshworks.com')
    ...
    upload_response = requests.post(upload_url, headers=headers, data=data_payload, files=files)
    logger.info("Respuesta de subida: %s %s", upload_response.status_code, upload_response.text)
    ...
    if upload_response.status_code in (200, 201):
        ...
        # Error en la subida
        logger.error("Error al subir el archivo a Freshsales: %s", response_json or upload_response.text)
...
def crear_actividad():
    try:
        ...
        campos_obligatorios = [url, db, username, password, res_id, activity_type_id, summary, date_deadline]
        ...
        if not res_model_id:
            res_model_data = models.execute_kw(db, uid, password, 'ir.model', 'search_read', [[['model', '=', res_model]]], {'fields': ['id']})
            if res_model_data:
                res_model_id = res_model_data[0]['id']
            else:
                return jsonify({'error': f"No se encontró el modelo '{res_model}' en Odoo."}), 400
        ...
        # Crear la actividad
        actividad_id = models.execute_kw(db, uid, password, 'mail.activity', 'create', [datos_actividad])

        return jsonify({'mensaje': f'Actividad creada con ID: {actividad_id}'}), 200
...
def leer_actividades():
    try:
        ...
        lead_exists = models.execute_kw(db, uid, password, 'crm.lead', 'search', [[['id', '=', res_id]]])
        if not lead_exists:
            return jsonify({'error': f"No se encontró el lead con ID {res_id}."}), 404

        # Obtener información del lead
        opportunity_data = models.execute_kw(db, uid, password, 'crm.lead', 'read', [res_id])
        ...
        if activity_ids:
            ...
            campos_actividades = ['create_date', 'summary', 'note', 'date_deadline']

            # Obtener información de las actividades con campos específicos
            activities_data = models.execute_kw(
                db, uid, password, 'mail.activity', 'read', [activity_ids],
            ...
            resultado_final = {
                "actividades": actividades_texto.strip(),             # Eliminar el último salto de línea
