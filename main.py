...
def send_message():
    ...
    thinking = data.get('thinking', 0)  # Nuevo parámetro con valor por defecto 0
    logger.info("subscriber_id recibido: %s", subscriber_id)
    logger.info("thinking recibido: %s", thinking)  # Log para verificar el valor de 'thinking'
    use_cache_control = data.get('use_cache_control', False)
    logger.info("Cache control es %s", use_cache_control)
    logger.info("Mensaje recibido: %s", message)  # Cambio en el log para mayor claridad
    ...
    variables.pop('thinking', None)  # Remover 'thinking' de las variables adicionales
